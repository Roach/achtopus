...
4. **Report with evidence.** Post to `bus/<id>.result.md`: the claim restated, the exact command(s) you ran, and the observed output (a fenced code block or a `$ `/`> ` command line — this is what lets the driver's evidence gate treat your pass as earned rather than downgrading it to `INCONCLUSIVE`). Note any deviation from the expected behavior and why.
