...
- Coordination happens through the **file wire** in `wire/`. Read `docs/protocol.md` for the
  file schema and task lifecycle, and `docs/patterns.md` for the four coordination patterns.
- Drive the manifest with `bin/wire` (`init`, `claim`, `status`, `manifest`, `watch`, `clear`).
...
- **Before claiming a task, read the manifest.** If it's already active, resume the owner via
  `SendMessage` — never spawn a duplicate. `bin/wire claim` guards this.
- Only `accepted` work (cleared by the tuner + heckler pair) enters the scribe's synthesis.

When coordinating a real task, the conductor writes `wire/plan.md`, assigns stable task ids,
